...
const chartMaker = {
    ...
    // 📊 차트 관련 데이터 만들기
    regionToSeries(rainData, region) {
...
export default Vue.extend({
    ...
    data() {
        return {
            rainData: {},
            regions: hadongParser.getCheckedList(),
            chartOptions: {},
            chartObj: {},
        };
    },
    ...
    methods: {
        ...
        setRegionChecked(region, checked) {
        ...
        addSeries(region) {
            ...
            const newSeries = chartMaker.regionToSeries(this.rainData, region);
            // 비구름 그리기
...
<style scoped>
.rain ul.selection > li {
    list-style-type: none;
    display: inline-block;
}
.rain ul.selection > li:not(:first-child) {
    margin-left: 15px;
}
</style>